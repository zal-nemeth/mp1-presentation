...
def find_fwhm(wavelengths, powers):
    ...
    f_left = interp1d(
        left_side[::-1], wl_left[::-1], bounds_error=False, fill_value="extrapolate"
    )
    f_right = interp1d(
        right_side, wl_right, bounds_error=False, fill_value="extrapolate"
    )
...
if __name__ == "__main__":
    ...
    wavelengths = np.linspace(
        centre_wavelength - (span / 2), centre_wavelength + (span / 2), int(samples)
    )

    # ➤ Find FWHM and -3 dB points
    peak_power, peak_wavelength, wl_low, wl_high, fwhm = find_fwhm(
        wavelengths, test_osa
    )
    ...
    if not np.isnan(fwhm):
        plt.axvline(
            wl_low, color="red", linestyle="--", label=f"-3 dB: {wl_low:.3f} nm"
        )
        plt.axvline(
            wl_high, color="red", linestyle="--", label=f"-3 dB: {wl_high:.3f} nm"
        )
        plt.axvline(
            peak_wavelength,
            color="blue",
