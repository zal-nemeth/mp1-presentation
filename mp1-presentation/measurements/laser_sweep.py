...
def agilent_sweep(
    ...
    number_data_points = (
        abs(int(math.floor((stop - start) / (resolution / 1000.0)))) + 1
    )
    if twow == 1:
        ls.write(":SOUR0:WAV:SWE:REP twow")
        ls.write(":sour0:wav:swe:cycl 2")
        number_data_points = (
            abs(int(math.floor(2 * (stop - start) / (resolution / 1000.0)))) + 1
        )
    ...
    wavelengthss = ls.query_binary_values(
        ":SOUR0:READ:DATA? LLOG", datatype="d", is_big_endian=False
    )
...
if __name__ == "__main__":
    ...
    w, p = agilent_sweep(
        start=1550, stop=1560, resolution=0.1, pwr=0.0, twow=0, sweep_s=10, sn=-30
    )
